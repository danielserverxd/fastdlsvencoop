...
void listsounds(CBasePlayer@ plr, const CCommand@ args) {
    ...
    g_PlayerFuncs.ClientPrint(plr, HUD_PRINTCONSOLE, "\nEscribe '.listsounds2' para ver audios adicionales que se pueden cargar de forma selectiva.\n\n");
}
...
void listsounds2(CBasePlayer@ plr, const CCommand@ args) {
    ...
    lines.insertLast("\nEstos audios deben seleccionarse con '.csload' antes de que puedan usarse.\n");
    lines.insertLast("Los sonidos descargados se pueden previsualizar con el comando '.cs' o transmitirlos de manera predeterminada con el comando '.csmic'.\n");
    lines.insertLast("Escribe '.listsounds' para ver la lista de sonidos que siempre se cargan.\n\n");
    ...
    if (pArguments.ArgC() > 0) {
        ...
        if (g_SoundList.exists(soundArg)) {
            ...
            if (d < g_Delay) {
                ...
                g_PlayerFuncs.ClientPrint(pPlayer, HUD_PRINTCENTER, "Espera " + format_float(w) + " segundos\n");
			...
			g_PlayerFuncs.SayText(pPlayer, "[Audios] Lista de audios enviado a tu consola.\n");
