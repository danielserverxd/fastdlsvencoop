...
class CMonsterRoy : ScriptBaseMonsterEntity
{
	...
	void Precache()
	{
		...
		if( string( self.pev.model ).IsEmpty() )
		{
			...
			//if( self.IsPlayerAlly() )
				g_Game.PrecacheModel("models/rede/roy.mdl");
			//else
			//	g_Game.PrecacheModel("models/barnabus.mdl");
		}

		g_SoundSystem.PrecacheSound("rede/roy/ro_attack1.ogg");
		g_SoundSystem.PrecacheSound("rede/roy/ro_attack2.ogg");
		g_SoundSystem.PrecacheSound("rede/roy/ro_pain1.ogg");
		g_SoundSystem.PrecacheSound("rede/roy/ro_pain2.ogg");
		g_SoundSystem.PrecacheSound("rede/roy/ro_pain3.ogg");
		g_SoundSystem.PrecacheSound("rede/roy/ro_die1.ogg");
		g_SoundSystem.PrecacheSound("rede/roy/ro_die2.ogg");
		g_SoundSystem.PrecacheSound("rede/roy/ro_die3.ogg");

		g_SoundSystem.PrecacheSound("rede/roy/stopthat.ogg");
		g_SoundSystem.PrecacheSound("rede/roy/stopthat.ogg");
	...
	bool KeyValue( const string& in szKey, const string& in szValue )
	{
		...
		{
			m_iszIsPlayerAlly = atoi(szValue);
			return true;
		}
		else 
		{
	...
	{
		//if( self.IsPlayerAlly() )
			g_EntityFuncs.SetModel( self, "models/rede/roy.mdl" );
		//else
	...
	{	
		...
		if((m_iszIsPlayerAlly == 1) && (pAttacker.pev.classname == "player"))
			return 0;
			...
			{		
				if( self.pev.deadflag == DEAD_NO )
				{
					...
					if( (self.m_afMemory & bits_MEMORY_SUSPICIOUS) != 0 || pAttacker.IsFacing( self.pev, 0.96f ) )
					{
						...
						self.PlaySentence( "RO_MAD", 4, VOL_NORM, ATTN_NORM );
					...
					else
					{
						...
						self.PlaySentence( "RO_SHOT", 4, VOL_NORM, ATTN_NORM );
	...
	void FearScream()
	{
		if( m_flNextFearScream < g_Engine.time )
		{
			...
			{
			case 0: g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_VOICE, "rede/roy/stopthat.ogg", 1, ATTN_NORM, 0, PITCH_NORM); break;
			case 1: g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_VOICE, "rede/roy/areyoumad.ogg", 1, ATTN_NORM, 0, PITCH_NORM); break;
			}
	...
	void PainSound()
	{
		...
		switch (Math.RandomLong(0,2))
		{
		case 0: g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_VOICE, "rede/roy/ro_pain1.ogg", 1, ATTN_NORM, 0, PITCH_NORM); break;
		case 1: g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_VOICE, "rede/roy/ro_pain2.ogg", 1, ATTN_NORM, 0, PITCH_NORM); break;
		case 2: g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_VOICE, "rede/roy/ro_pain3.ogg", 1, ATTN_NORM, 0, PITCH_NORM); break;
		}
	}
	...
	void DeathSound()
	{
		switch (Math.RandomLong(0,2))
		{
		case 0: g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_VOICE, "rede/roy/ro_die1.ogg", 1, ATTN_NORM, 0, PITCH_NORM); break;
		case 1: g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_VOICE, "rede/roy/ro_die2.ogg", 1, ATTN_NORM, 0, PITCH_NORM); break;
		case 2: g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_VOICE, "rede/roy/ro_die3.ogg", 1, ATTN_NORM, 0, PITCH_NORM); break;
		}
	}
	...
	Schedule@ GetSchedule()
	{
		...
		if( self.HasConditions( bits_COND_ENEMY_DEAD ) )
			self.PlaySentence( "RO_KILL", 4, VOL_NORM, ATTN_NORM );
	...
	void FollowerUse( CBaseEntity@ pActivator, CBaseEntity@ pCaller, USE_TYPE useType, float flValue )
	{
		...
		if ( self.pev.SpawnFlagBitSet( 256 )) //pre disaster
		{
			g_SoundSystem.PlaySentenceGroup( self.edict(), "RO_POK", 1.0, ATTN_NORM, 0, PITCH_NORM );
			return;
		}
		
		if( pTarget is pActivator ) 
		{
			if( self.IsPlayerFollowing() )
			{
				g_SoundSystem.PlaySentenceGroup( self.edict(), "RO_OK", 1.0, ATTN_NORM, 0, PITCH_NORM );
			}
		}
		else
		{	
			g_SoundSystem.PlaySentenceGroup( self.edict(), "RO_WAIT", 1.0, ATTN_NORM, 0, PITCH_NORM );
			self.StopPlayerFollowing(true,false);
		}
	}
}
...
void RegisterMonsterRoy()
{
	InitSchedules();
	g_CustomEntityFuncs.RegisterCustomEntity( "CMonsterRoy", "monster_roy" );
}
