...
{
	return args.Arg(0).ToLowercase() == "b" 
		|| args.Arg(0).ToLowercase() == "/b" 
		|| args.Arg(0).ToLowercase() == "!b" 
		|| args.Arg(0).ToLowercase() == ".b" 
		|| args.Arg(0).ToLowercase() == "\\b" 
		|| args.Arg(0).ToLowercase() == "#b";
...
CClientCommand _buy( "b", "Opens the BuyMenu", @INS2_Buy );
	...
	else if( args.ArgC() == 3 )
	{
		...
		if( g_Ins2Menu.m_Items.length() > 0 )
		{
			...
			if( bItemFound )
			{
				...
				{
					g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTCONSOLE, "[INS2 Tienda] No hay suficiente dinero para comprar: " + szItemName + " - Costo: $" + uiCost + "\n" );
				}
				else if( pPlayer.HasNamedPlayerItem( szItemName ) !is null )
				{
					g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTCONSOLE, "[INS2 Tienda] Ya tienes esta arma: " + szItemName + "\n" );
				}
				else
				{ 
			...
			if( pPlayer.m_hActiveItem.GetEntity() !is null )
			{
				...
				if( bItemFound )
				{
					if( uint(BuyMenu::BuyPoints[BuyMenu::PlayerID( pPlayer )]) <= 0 )
					{
						...
						if( bItem2Found )
						{
							g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTCONSOLE, "[INS2 Tienda] No hay suficiente dinero para comprar: " + szItem2Name + " - Costo: $" + uiCost2 + "\n" );
						}
					}
					else
					{ 
...
HookReturnCode INS2_ClientSay( SayParameters@ pParams )
{
	...
	else if( args.ArgC() == 3 && FirstArgChecker( args ) )
	{
		...
		{
			g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[INS2 Tienda] Tipo de objeto no valido\n" );
			return HOOK_CONTINUE;
		}
		...
		if( g_Ins2Menu.m_Items.length() > 0 )
		{
			...
			if( bItemFound )
			{
				if( uint(BuyMenu::BuyPoints[BuyMenu::PlayerID( pPlayer )]) <= 0 )
				{
					g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[INS2 Tienda] No hay suficiente dinero para comprar: " + szItemName + " - Costo: $" + uiCost + "\n" );
				}
				else if( pPlayer.HasNamedPlayerItem( szItemName ) !is null )
				{
					g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[INS2 Tienda] Ya tienes esta arma: " + szItemName + "\n" );
				}
				else
				{ 
					if( uint(BuyMenu::BuyPoints[BuyMenu::PlayerID( pPlayer )]) >= uiCost )
			...
			if( pPlayer.m_hActiveItem.GetEntity() !is null )
			{
				...
				if( bItemFound )
				{
					if( uint(BuyMenu::BuyPoints[BuyMenu::PlayerID( pPlayer )]) <= 0 )
					{
						...
						if( bItem2Found )
						{
							g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[INS2 Tienda] No hay suficiente dinero para comprar: " + szItem2Name + " - Costo: $" + uiCost2 + "\n" );
						}
					}
					else
					{ 
...
final class BuyableItem
{
	...
	private void GiveItem( CBasePlayer@ pPlayer ) const
	{
		if( uint(BuyPoints[PlayerID( pPlayer )]) < m_uiCost )
		{
			...
			g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[INS2 Tienda] No hay suficiente dinero para comprar: " + m_szEntityName + " - Costo: $" +  m_uiCost + "\n" );
			return;
		}
		...
		if( pPlayer.HasNamedPlayerItem( m_szEntityName ) !is null )
		{
			...
			if( !(pPlayer.HasNamedPlayerItem( m_szEntityName ).iFlags() < 0) && pPlayer.HasNamedPlayerItem( m_szEntityName ).iFlags() & ITEM_FLAG_EXHAUSTIBLE != 0 )
			{
				if( pPlayer.GiveAmmo( pPlayer.HasNamedPlayerItem( m_szEntityName ).GetWeaponPtr().m_iDefaultAmmo, m_szEntityName, pPlayer.GetMaxAmmo( m_szEntityName ) ) != -1 )
				{
					...
					g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[INS2 Tienda] Compraste municion para: " + m_szEntityName + "\n" );
				}
				else
				{
					g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[INS2 Tienda] Ya tienes municion maxima para esta arma!\n" );
					return;
				}
			}
			else
			{
				g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[INS2 Tienda] Ya tienes ese articulo!\n" );
				return;
			}
			//KernCore end
		}
		...
		{
			g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[INS2 Tienda] Dinero insuficiente - Costo: $" + m_uiCost + "\n");
			return;
		}
	}
}
...
final class BuyMenu
{
	...
	private void CreateMenu()
	{
		...
		m_pPrimaryMenu.SetTitle( sChoose + " la categoria de arma principal: " );
		...
		m_pSecondaryMenu.SetTitle( sChoose + " la categoria de arma secundaria: " );
	...
	private void MainCallback( CTextMenu@ menu, CBasePlayer@ pPlayer, int iSlot, const CTextMenuItem@ pItem )
	{
		if( pItem !is null )
		{
			...
			if( sChoice == "Tienda de Armas Primarias" )
