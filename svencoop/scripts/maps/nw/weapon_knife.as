...
class weapon_knife : ScriptBasePlayerWeaponEntity
{
	...
	bool Swing( int fFirst )
	{
		...
		else
		{
			...
			float flDamage = g_EngineFuncs.CVarGetFloat("sk_plr_crowbar") > 10 ? g_EngineFuncs.CVarGetFloat("sk_plr_crowbar") : 10;
			...
			if ( self.m_flNextPrimaryAttack + 1 < g_Engine.time )
			{
				...
				pEntity.TraceAttack( m_pPlayer.pev, flDamage * 2, g_Engine.v_forward, tr, DMG_CLUB );  
			}
			else
			{
	...
	bool HeavySmack()
	{
		...
		Math.MakeVectors( m_pPlayer.pev.v_angle );
		Vector vecSrc	= m_pPlayer.GetGunPosition();
		Vector vecEnd	= vecSrc + g_Engine.v_forward * 65;
		...
		else
		{
			...
			{
				g_WeaponFuncs.ClearMultiDamage();
				float flDamage = g_EngineFuncs.CVarGetFloat("sk_plr_crowbar") * 3.5 > 35 ? g_EngineFuncs.CVarGetFloat("sk_plr_crowbar") * 3.5 : 35;
...
class knife_throw : ScriptBaseEntity
{
	...
	void Touch( CBaseEntity@ pOther )
	{
		...
		if( pOther.pev.takedamage > DAMAGE_NO )
		{
			...
			g_WeaponFuncs.ClearMultiDamage();
			pOther.TraceAttack( pev.owner.vars, flDamage, g_Engine.v_forward, tr, DMG_NEVERGIB );  
