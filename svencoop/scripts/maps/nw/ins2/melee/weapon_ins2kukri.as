...
string V_MODEL = "models/nw/ins2/wpn/kukri/v_kukri.mdl";
...
int DEFAULT_GIVE 	= 0;
int WEIGHT      	= 20;
int FLAGS       	= ITEM_FLAG_ESSENTIAL;
uint DAMAGE      	= 100; //50
uint SLOT       	= 0;
uint POSITION   	= 9;
string AMMO_TYPE 	= "";

class weapon_ins2kukri : ScriptBasePlayerWeaponEntity, NWINS2BASE::WeaponBase, NWINS2BASE::MeleeWeaponBase
{
	...
	void Precache()
	{
		...
		NWINS2BASE::PrecacheSound( NWINS2BASE::DeployBayonetSounds );
		NWINS2BASE::PrecacheSound( KnifeHitFlesh );
		NWINS2BASE::PrecacheSound( BayoHitWorld );
		NWINS2BASE::PrecacheSound( SwingMelee );
	...
	{
		PlayDeploySound( 0 );
		return Deploy( V_MODEL, P_MODEL, DRAW, "crowbar", GetBodygroup(), (19.0/40.0) );
	}
	...
	void PrimaryAttack()
	{
		...
		self.m_flTimeWeaponIdle = self.m_flNextPrimaryAttack = WeaponTimeBase() + (26.0/40.0);
		SetThink( ThinkFunction( this.DoHeavyAttack ) );
		self.pev.nextthink = g_Engine.time + (4.5/40.0);
	}

	void DoHeavyAttack()
	{
		Smack( 46.03834, (16.0/40.0), DAMAGE, (26.0/40.0), KnifeHitFlesh[Math.RandomLong( 0, KnifeHitFlesh.length() - 1)], BayoHitWorld[Math.RandomLong( 0, BayoHitWorld.length() - 1)], DMG_SLASH | DMG_CLUB );
