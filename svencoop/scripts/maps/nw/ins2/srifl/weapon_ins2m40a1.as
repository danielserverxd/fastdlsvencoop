...
string V_MODEL = "models/nw/ins2/wpn/m40a1/v_m40a1.mdl";
...
uint DAMAGE     	= 100; //100 * 1.8 = 180
uint SLOT       	= 7;
uint POSITION   	= 4;
float RPM       	= 1.50f; //Rounds per minute in air
string AMMO_TYPE 	= "ins2_7.62x51mm";

class weapon_ins2m40a1 : ScriptBasePlayerWeaponEntity, NWINS2BASE::WeaponBase
{
	...
	{
		info.iMaxAmmo1 	= (NWINS2BASE::ShouldUseCustomAmmo) ? MAX_CARRY : NWINS2BASE::DF_MAX_CARRY_M40A1;
	...
	bool Deploy()
	{
		...
		{
			m_WasDrawn = true;
			return Deploy( V_MODEL, P_MODEL, DRAW_FIRST, "sniper", GetBodygroup(), (70.0/35.0) );
		}
		else
		{
			if( m_bCanBolt )
			{
				...
				self.pev.nextthink = g_Engine.time + (28.0/34.0);
			}

			return Deploy( V_MODEL, P_MODEL, (self.m_iClip <= 0 || m_bCanBolt ) ? DRAW_EMPTY : DRAW, "sniper", GetBodygroup(), (29.0/34.0) );
		}
	}
	...
	void PrimaryAttack()
	{
		...
		ShootWeapon( SHOOT_S, 1, VecModAcc( VECTOR_CONE_1DEGREES, 1.15f, 0.15f, 1.15f ), (m_pPlayer.pev.waterlevel == WATERLEVEL_HEAD) ? 1024 : 16384, DAMAGE, true, DMG_SNIPER | DMG_POISON, 0.5, 0.8 );
	...
	void ShellEjectThink() //For the think function used in shoot
	{
		m_bCanBolt = false;
		SetThink( null );
		ShellEject( m_pPlayer, m_iShell, (WeaponADSMode != NWINS2BASE::IRON_IN) ? Vector( 20, 7, -9 ) : Vector( 20, 2, -9 ) );
	}

	void SightThink()
	{
		ToggleZoom( 15 );
		WeaponADSMode = NWINS2BASE::IRON_IN;
		SetPlayerSpeed();
	...
	void SecondaryAttack()
	{
		...
		g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_VOICE, NWINS2BASE::AimDownSights_in[ Math.RandomLong( 0, NWINS2BASE::AimDownSights_in.length() - 1 )], VOL_NORM, ATTN_NORM, 0, PITCH_NORM );
	...
	void ItemPostFrame()
	{
		...
		if( m_fSniperReload )
		{
			if( (CheckButton() || (self.m_iClip >= MAX_CLIP && m_pPlayer.pev.button & IN_RELOAD != 0)) && m_flNextReload <= g_Engine.time )
			{
				...
				self.m_flTimeWeaponIdle = g_Engine.time + (31.0/31.0);
				self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = g_Engine.time + (28.0/31.0);
				m_fSniperReload = false;
			}
		}
	...
	void Reload()
	{
		...
		if( m_reloadTimer < g_Engine.time )
		{
			...
			if( !m_fSniperReload )
			{
				...
				self.pev.nextthink = WeaponTimeBase() + (22.0/35.0);

				m_pPlayer.m_flNextAttack = (39.0/35.0); //Always uses a relative time due to prediction
				self.m_flTimeWeaponIdle = self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (39.0/35.0);
			...
			else if( m_fSniperReload )
			{
				...
				m_flNextReload = WeaponTimeBase() + (39.0/60.0);
				self.m_flTimeWeaponIdle = self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + (39.0/60.0);
...
class M40A1_MAG : ScriptBasePlayerAmmoEntity, NWINS2BASE::AmmoBase
{
	void Spawn()
	{
		Precache();
	...
	{
		return CommonAddAmmo( pOther, MAX_CLIP * 4, (NWINS2BASE::ShouldUseCustomAmmo) ? MAX_CARRY : NWINS2BASE::DF_MAX_CARRY_M40A1, (NWINS2BASE::ShouldUseCustomAmmo) ? AMMO_TYPE : NWINS2BASE::DF_AMMO_M40A1 );
	}
}
...
void Register()
{
	...
	g_ItemRegistry.RegisterWeapon( GetName(), SPR_CAT, (NWINS2BASE::ShouldUseCustomAmmo) ? AMMO_TYPE : NWINS2BASE::DF_AMMO_M40A1, "", GetAmmoName() ); // Register the weapon
}
