...
mixin class WeaponBase
{
	...
	{
		// Arrays
		NWINS2BASE::PrecacheSound( AimDownSights_in );
		NWINS2BASE::PrecacheSound( AimDownSights_out );
	...
	{ // LCH: DefaultReload function is shit, need rewrite.
		//self.m_fInReload = true;
		//self.DefaultReload( iAmmo, iAnim, iTimer, iBodygroup );
		//self.m_flTimeWeaponIdle = WeaponTimeBase() + iTimer;
		ReloadPrimaryLCH( iAmmo, iAnim, iTimer, iBodygroup );
		m_iShotsFired = 0;
	}

	private int SavePrimaryAmmo, ReloadedPrimaryAmmo, CurrentMagPrimaryAmmo;
	void ReloadPrimaryLCH( int iAmmo, int iAnim, float flTimer, int iBodygroup ) //sets timer to remove primary ammo
	{
		if( self.m_fInReload )
			return;
		...
		SetThink( ThinkFunction( this.RemovePrimaryAmmoLCH ) );
		self.pev.nextthink = g_Engine.time + flTimer - 0.2;
	}

	void RemovePrimaryAmmoLCH() // removes primary ammo
	{
		SetThink( null );
		SavePrimaryAmmo = m_pPlayer.m_rgAmmo( self.m_iPrimaryAmmoType );
		CurrentMagPrimaryAmmo = self.m_iClip;
		int shouldReloadAmmo = Math.min(ReloadedPrimaryAmmo - CurrentMagPrimaryAmmo, SavePrimaryAmmo);
		self.m_iClip += shouldReloadAmmo;
		SavePrimaryAmmo -= shouldReloadAmmo;

		self.m_fInReload = false;
		m_pPlayer.m_rgAmmo( self.m_iPrimaryAmmoType, SavePrimaryAmmo );
	}
	...
	void ShootWeapon( const string Sound, const uint numShots, Vector& in CONE, float maxDist, int Damage, const bool shouldTrace = false, const int DmgType = DMG_GENERIC, float WaterBonus = 0.5, float traceBonus = 0, bool isSecondary = false ) // pew pew
	{
		if( Sound != string_t() || Sound != "" )
		{
			if(isSecondary){
				if( self.m_iClip2 != 0 && self.iMaxClip() != WEAPON_NOCLIP )
					--self.m_iClip2;
		...
		m_pPlayer.FireBullets( numShots, vecSrc, vecAiming, VECTOR_CONE_1DEGREES, maxDist, BULLET_PLAYER_CUSTOMDAMAGE, 2, (m_pPlayer.pev.waterlevel == WATERLEVEL_HEAD) ? int(Damage * WaterBonus) : Damage );

		if( (IsWeaponEmpty() && m_pPlayer.m_rgAmmo( self.m_iPrimaryAmmoType ) <= 0) && (IsWeaponEmpty() && m_pPlayer.m_rgAmmo( self.m_iSecondaryAmmoType ) <= 0) )
			...
			if( tr.flFraction < 1.0 )
			{
				if( tr.pHit !is null )
				{
					...
					if( DmgType != DMG_GENERIC )
					{
						if( pHit !is null )
						{
							int DamageType = DmgType;
							if( traceBonus <= 0 )
								traceBonus = (DmgType & DMG_BLAST != 0) ? 0.2 : 0.3636;
							if(pHit.IsBSPModel() == true || pHit.IsMachine()){
								array<string> HeliCopter = {"monster_apache","monster_osprey","monster_blkop_apache","monster_blkop_osprey"};
								bool isHeliCopter = HeliCopter.find(pHit.pev.classname) >= 0;
								bool isRoboGrunt = pHit.pev.classname == "monster_robogrunt";
								bool isIslave = pHit.pev.classname == "monster_alien_slave";
								bool isElectric = DmgType & DMG_SHOCK != 0;
								bool isSpecialAP = DmgType & DMG_SNIPER != 0 && DmgType & (~(DMG_SNIPER | DMG_NEVERGIB | DMG_ALWAYSGIB)) != 0;
								if(isHeliCopter && (isSpecialAP || isElectric)){
									traceBonus = traceBonus * 2 < traceBonus + 1 ? traceBonus * 2 : traceBonus + 1 ;
									if(isElectric)
										DamageType = DMG_SHOCK;
									else
										DamageType = DMG_SNIPER;
								}
								else if(isRoboGrunt && (isSpecialAP || isElectric)){
									if(isElectric)
										DamageType = DMG_SHOCK;
									else
										DamageType = DMG_LAUNCH;
								}
								else if(isElectric && isIslave)
									DamageType &= ~DMG_SHOCK;
								else if(DmgType & DMG_POISON != 0)
									DamageType = DmgType & (~DMG_POISON); //Posion Damage only cause 10% damage for bspmodel
							}
							g_WeaponFuncs.ClearMultiDamage();
							pHit.TraceAttack( m_pPlayer.pev, Damage * traceBonus, vecEnd, tr, DamageType );
	...
	void DispFastReloadTxt() // displays text on the player's hud so he knows how to change the firemode
	{
		HUDTextParams params;
		params.channel = 4;
		params.fadeinTime = 0;
		...
		params.b1 = RGBA_SVENCOOP.b;
		params.a1 = 255;
		g_PlayerFuncs.HudMessage( m_pPlayer, params, "Can FastReload" );
	}
