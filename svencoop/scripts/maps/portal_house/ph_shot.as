...
class CBlaster : ScriptBaseEntity
{
	...
	void Precache()
	{
		m_iExplode = g_Game.PrecacheModel( "sprites/portal_house/expB1.spr" );
		m_iSpriteTexture = g_Game.PrecacheModel( "sprites/portal_house/plasma.spr" );
		...
		m_blasterBolt = g_Game.PrecacheModel( "sprites/portal_house/plasma.spr" );
		m_blasterImpact = g_Game.PrecacheModel( "sprites/portal_house/blasterimpact.spr" );
	...
	void BlasterLight()
	{
		...
		@m_pBeam = g_EntityFuncs.CreateBeam( "sprites/portal_house/plasma.spr", 15 );
...
void te_sprite(Vector pos, string sprite="sprites/portal_house/blasterimpact.spr", 
...
class CHPBlaster : ScriptBaseEntity
{
	...
	void BlasterLight()
	{
		...
		Math.MakeAimVectors( pev.angles );
		@m_pBeam = g_EntityFuncs.CreateBeam( "sprites/portal_house/plasma.spr", 25 );
		m_pBeam.SetStartPos( self.pev.origin + g_Engine.v_forward * 10 );
		m_pBeam.SetEndPos( self.pev.origin - g_Engine.v_forward * 90 );
		m_pBeam.SetColor( 255, 255, 179 );
		m_pBeam.SetScrollRate( 0 );
...
class CCShoot : ScriptBaseEntity
{
	...
	void Spawn()
	{
		...
		self.pev.solid = SOLID_BBOX;
		g_EntityFuncs.SetModel( self, "models/not_precached.mdl" );
		self.pev.body = 1;
		m_yawCenter = pev.angles.y;
		m_pitchCenter = pev.angles.x;
		g_EntityFuncs.SetSize( self.pev, g_vecZero, g_vecZero );
		
		@cShoot = g_EntityFuncs.CreateSprite( "sprites/portal_house/cshoot.spr", self.pev.origin, false ); 
	...
	void Precache()
	{
		...
		m_iSmoke = g_Game.PrecacheModel( "sprites/steam1.spr" );
		m_iTrail = g_Game.PrecacheModel( "sprites/smoke.spr" );
		m_iSpriteTextureShoot = g_Game.PrecacheModel( "sprites/portal_house/cshoot.spr" );
	}
	...
	void Explode( CBaseEntity@ pOther, TraceResult pTrace, int bitsDamageType )
	{
		...
		ProjectileEffect( self.pev.origin );
		g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_WEAPON, "weapons/mortarhit.wav", 0.4, ATTN_LOW_HIGH, 0, PITCH_NORM );
	...
	void ProjectileEffect( Vector origin )
	{
		...
		NetworkMessage projectilexp3( MSG_PVS, NetworkMessages::SVC_TEMPENTITY );
			projectilexp3.WriteByte( TE_BEAMDISK );
		...
		NetworkMessage projectilexp5( MSG_PVS, NetworkMessages::SVC_TEMPENTITY );
			projectilexp5.WriteByte( TE_BEAMDISK );
...
class CBlasterNpc : ScriptBaseEntity
{
	...
	void Explode( CBaseEntity@ pOther, TraceResult pTrace, int bitsDamageType )
	{
		...
		ProjectileEffect( self.pev.origin );
		g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_WEAPON, "buttons/latchunlocked2.wav", 0.4, ATTN_LOW_HIGH, 0, PITCH_NORM );
...
CBlaster@ ShootBlaster( entvars_t@ pevOwner, Vector vecStart, Vector vecVelocity )
{
	...
	ProjectileDynamicLight( pProjectile.pev.origin, 6, 255, 255, 255, 5, 50 );
...
CBlaster@ ShootBlasterCrossbow2( entvars_t@ pevOwner, Vector vecStart, Vector vecVelocity )
{
	...
	@pProjectile.pev.owner = pevOwner.pContainingEntity;
	pProjectile.SetTouch( TouchFunction( pProjectile.ExplodeTouch ) );
	pProjectile.pev.dmg = BOWCASTER_DAMAGE * 10;
	ProjectileDynamicLight( pProjectile.pev.origin, 6, 255, 255, 255, 5, 50 );
...
CBlasterNpc@ ShootBlasterNpc( int damageBlaster, entvars_t@ pevOwner, Vector vecStart, Vector vecVelocity )
{
	...
	ProjectileDynamicLight( pProjectile.pev.origin, 10, 255, 255, 255, 5, 40 );	
