void MapSaveRegister(const string strSaveMapList)
{
	...
	if( strSaveMapList == "" )
		return;
	// Cleanup routine for savestates - remove save files when these maps are NOT running
	...
	if( STR_SAVE_MAPS.find( g_Engine.mapname ) < 0 )
		for( uint i = 0; i < STR_SAVE_MAPS.length(); i++ )
			g_FileSystem.RemoveFile( "scripts/maps/store/" + STR_SAVE_MAPS[i] + ".dat" );
}
