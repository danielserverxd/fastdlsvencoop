...
namespace POKECHECKPOINT {

const string g_telespr = "sprites/e-tele1.spr";
const string g_telemdl = "models/blackops/misc/omegared.mdl";
const string g_survsnd = "blackops/misc/checkpoint_survival.wav";
const string g_pulssnd = "blackops/misc/checkpoint_pulse.wav";
const string g_poclsnd = "blackops/misc/checkpoint_close.wav";
...
class point_checkpoint : ScriptBaseAnimating
{
	...
	private float m_flDelayBeforeStart 			= 6;
	...
	void Precache()
	{
		...
		if( string( self.pev.model ).IsEmpty() )
			g_Game.PrecacheModel( g_telemdl );
		else
			g_Game.PrecacheModel( self.pev.model );
		
		g_Game.PrecacheModel( g_telespr );
		
		g_SoundSystem.PrecacheSound( g_survsnd );
		g_SoundSystem.PrecacheSound( g_pulssnd );
		g_SoundSystem.PrecacheSound( g_poclsnd );
		
		if( string( self.pev.message ).IsEmpty() )
			self.pev.message = g_pulssnd;
	...
	void Spawn()
	{
		...
		// Allow for custom models
		if( string( self.pev.model ).IsEmpty() )
			g_EntityFuncs.SetModel( self, g_telemdl );
		else
	...
	void Touch( CBaseEntity@ pOther )
	{
		...
		g_SoundSystem.EmitSound( self.edict(), CHAN_STATIC, g_survsnd, 1.0f, ATTN_NONE );
	...
	void RespawnStartThink()
	{
		...
		@m_pSprite = g_EntityFuncs.CreateSprite( g_telespr, self.pev.origin, true, 10 );
		m_pSprite.TurnOn();
		m_pSprite.pev.rendermode = kRenderTransAdd;
		m_pSprite.pev.renderamt = 128;
	
		g_SoundSystem.EmitSound( self.edict(), CHAN_STATIC, g_pulssnd, 1.0f, ATTN_NORM );
	...
	{
		g_SoundSystem.EmitSound( self.edict(), CHAN_STATIC, g_poclsnd, 1.0f, ATTN_NORM );
...
{
	g_CustomEntityFuncs.RegisterCustomEntity( "POKECHECKPOINT::point_checkpoint", "point_checkpoint" );
}
