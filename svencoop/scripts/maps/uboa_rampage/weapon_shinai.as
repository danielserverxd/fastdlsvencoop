...
class weapon_shinai : weapon_uboamelee {
    void Spawn() {
        Init("shinai");
    }
	...
	void PrimaryAttack() {
	    ...
	    float dmgBuf = 27.0 + Math.RandomFloat(0.0, 3.0) + (0.5 * mRageLevel);
...
string GetShinaiName() {
	return "weapon_shinai";
}

void RegisterShinai() {
	g_CustomEntityFuncs.RegisterCustomEntity( "weapon_shinai", GetShinaiName() );
	g_ItemRegistry.RegisterWeapon( GetShinaiName(), "uboa_rampage", DURABILITY_AMMO_NAME, UBOAKIAI_AMMO_NAME);
}
