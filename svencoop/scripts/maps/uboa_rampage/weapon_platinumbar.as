...
class weapon_platinumbar : weapon_uboamelee {
	...
	void PrimaryAttack() {
	    ...
	    float dmgBuf = 40.0 + Math.RandomFloat(0.0, 3.0) + (0.5 * mRageLevel);
	...
	void SecondaryAttack() {
	    // Rageモードと合わせてスピード、威力調整
	    float dmgBuf = 60.0 + (0.6 * mRageLevel);
	    float spdBuf = (mRageLevel > 0) ? 0.3 : 0.35;
...
string GetPlatinumbarName() {
	return "weapon_platinumbar";
}

void RegisterPlatinumbar() {
	g_CustomEntityFuncs.RegisterCustomEntity( "weapon_platinumbar", GetPlatinumbarName() );
	g_ItemRegistry.RegisterWeapon( GetPlatinumbarName(), "uboa_rampage", DURABILITY_AMMO_NAME, UBOAKIAI_AMMO_NAME);
}
