...
class weapon_metalbat : weapon_uboamelee {
	...
	void PrimaryAttack() {
	    ...
	    float dmgBuf = 32.0 + Math.RandomFloat(0.0, 3.0) + (0.5 * mRageLevel);
	    float spdBuf = (mRageLevel > 0) ? 0.2 : 0.53;
	...
	void SecondaryAttack() {
	    // Rageモードと合わせてスピード、威力調整
	    float dmgBuf = 48.0 + (0.6 * mRageLevel);
	    float spdBuf = (mRageLevel > 0) ? 0.3 : 0.53;
...
string GetMetalbatName() {
	return "weapon_metalbat";
}

void RegisterMetalbat() {
	g_CustomEntityFuncs.RegisterCustomEntity( "weapon_metalbat", GetMetalbatName() );
	g_ItemRegistry.RegisterWeapon( GetMetalbatName(), "uboa_rampage", DURABILITY_AMMO_NAME, UBOAKIAI_AMMO_NAME);
}
