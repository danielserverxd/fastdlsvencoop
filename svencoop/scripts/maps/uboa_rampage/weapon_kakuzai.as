...
class weapon_kakuzai : weapon_uboamelee {
	...
	void PrimaryAttack() {
	    ...
	    float spdBuf = (mRageLevel > 0) ? 0.2 : 0.37;
	...
	void SecondaryAttack() {
	    // Rageモードと合わせてスピード、威力調整
	    float dmgBuf = 39.0 + (0.6 * mRageLevel);
	    float spdBuf = (mRageLevel > 0) ? 0.3 : 0.37;
...
string GetKakuzaiName() {
	return "weapon_kakuzai";
}

void RegisterKakuzai() {
	g_CustomEntityFuncs.RegisterCustomEntity( "weapon_kakuzai", GetKakuzaiName() );
	g_ItemRegistry.RegisterWeapon( GetKakuzaiName(), "uboa_rampage", DURABILITY_AMMO_NAME, UBOAKIAI_AMMO_NAME);
}
