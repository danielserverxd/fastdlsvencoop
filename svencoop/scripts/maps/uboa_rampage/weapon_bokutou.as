...
class weapon_bokutou : weapon_uboamelee {
	...
	void PrimaryAttack() {
	    ...
	    float dmgBuf = 28.0 + Math.RandomFloat(0.0, 3.0) + (0.5 * mRageLevel);
	    float spdBuf = (mRageLevel > 0) ? 0.2 : 0.47;
	...
	void SecondaryAttack() {
	    // Rageモードと合わせてスピード、威力調整
	    float dmgBuf = 44.0 + (0.6 * mRageLevel);
	    float spdBuf = (mRageLevel > 0) ? 0.3 : 0.47;
...
string GetBokutouName() {
	return "weapon_bokutou";
}

void RegisterBokutou() {
	g_CustomEntityFuncs.RegisterCustomEntity( "weapon_bokutou", GetBokutouName() );
	g_ItemRegistry.RegisterWeapon( GetBokutouName(), "uboa_rampage", DURABILITY_AMMO_NAME, UBOAKIAI_AMMO_NAME);
}
