...
class weapon_tennisracket : weapon_uboamelee {
    void Spawn() {
        Init("tennisracket");
    }
	...
	void PrimaryAttack() {
	    ...
	    float dmgBuf = 23.0 + Math.RandomFloat(0.0, 3.0) + (0.5 * mRageLevel);
	    float spdBuf = (mRageLevel > 0) ? 0.2 : 0.33;
	...
	void SecondaryAttack() {
	    // Rageモードと合わせてスピード、威力調整
	    float dmgBuf = 36.0 + (0.6 * mRageLevel);
	    float spdBuf = (mRageLevel > 0) ? 0.3 : 0.33;
...
string GetTennisRacketName() {
	return "weapon_tennisracket";
}

void RegisterTennisRacket() {
	g_CustomEntityFuncs.RegisterCustomEntity( "weapon_tennisracket", GetTennisRacketName() );
	g_ItemRegistry.RegisterWeapon( GetTennisRacketName(), "uboa_rampage", DURABILITY_AMMO_NAME, UBOAKIAI_AMMO_NAME);
}
