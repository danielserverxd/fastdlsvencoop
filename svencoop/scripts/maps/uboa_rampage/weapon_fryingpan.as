...
class weapon_fryingpan : weapon_uboamelee {
	...
	void PrimaryAttack() {
	    ...
	    float dmgBuf = 30.0 + Math.RandomFloat(0.0, 3.0) + (0.5 * mRageLevel);
	    float spdBuf = (mRageLevel > 0) ? 0.2 : 0.52;
	...
	void SecondaryAttack() {
	    // Rageモードと合わせてスピード、威力調整
	    float dmgBuf = 45.0 + (0.6 * mRageLevel);
	    float spdBuf = (mRageLevel > 0) ? 0.3 : 0.52;
...
string GetFryingpanName() {
	return "weapon_fryingpan";
}

void RegisterFryingpan() {
	g_CustomEntityFuncs.RegisterCustomEntity( "weapon_fryingpan", GetFryingpanName() );
	g_ItemRegistry.RegisterWeapon( GetFryingpanName(), "uboa_rampage", DURABILITY_AMMO_NAME, UBOAKIAI_AMMO_NAME);
}
