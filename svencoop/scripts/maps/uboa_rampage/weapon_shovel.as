...
class weapon_shovel : weapon_uboamelee {
	...
	void PrimaryAttack() {
	    ...
	    float dmgBuf = 35.0 + Math.RandomFloat(0.0, 3.0) + (0.5 * mRageLevel);
	    float spdBuf = (mRageLevel > 0) ? 0.2 : 0.55;
	...
	void SecondaryAttack() {
	    // Rageモードと合わせてスピード、威力調整
	    float dmgBuf = 52.0 + (0.6 * mRageLevel);
	    float spdBuf = (mRageLevel > 0) ? 0.3 : 0.55;
...
string GetShovelName() {
	return "weapon_shovel";
}

void RegisterShovel() {
	g_CustomEntityFuncs.RegisterCustomEntity( "weapon_shovel", GetShovelName() );
	g_ItemRegistry.RegisterWeapon( GetShovelName(), "uboa_rampage", DURABILITY_AMMO_NAME, UBOAKIAI_AMMO_NAME);
}
