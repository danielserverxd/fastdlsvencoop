...
class weapon_monkeywrench : weapon_uboamelee {
    void Spawn() {
        Init("monkeywrench");
    }
	...
	void PrimaryAttack() {
	    ...
	    float dmgBuf = 25.0 + Math.RandomFloat(0.0, 3.0) + (0.5 * mRageLevel);
	    float spdBuf = (mRageLevel > 0) ? 0.2 : 0.42;
	...
	void SecondaryAttack() {
	    // Rageモードと合わせてスピード、威力調整
	    float dmgBuf = 40.0 + (0.6 * mRageLevel);
	    float spdBuf = (mRageLevel > 0) ? 0.3 : 0.42;
...
string GetMonkeyWrenchName() {
	return "weapon_monkeywrench";
}

void RegisterMonkeyWrench() {
	g_CustomEntityFuncs.RegisterCustomEntity( "weapon_monkeywrench", GetMonkeyWrenchName() );
	g_ItemRegistry.RegisterWeapon( GetMonkeyWrenchName(), "uboa_rampage", DURABILITY_AMMO_NAME, UBOAKIAI_AMMO_NAME);
}
